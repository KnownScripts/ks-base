local disableHudComponents = QBConfig.Disable.disableHudComponents
local disableControls = QBConfig.Disable.disableControls
local displayAmmo = QBConfig.Disable.displayAmmo

local function DecorSet(Type, Value)
    if Type == 'parked' then
        QBConfig.Density['parked'] = Value
    elseif Type == 'vehicle' then
        QBConfig.Density['vehicle'] = Value
    elseif Type == 'multiplier' then
        QBConfig.Density['multiplier'] = Value
    elseif Type == 'peds' then
        QBConfig.Density['peds'] = Value
    elseif Type == 'scenario' then
        QBConfig.Density['scenario'] = Value
    end
end
...
CreateThread(function()
    while true do
        ...
        SetParkedVehicleDensityMultiplierThisFrame(QBConfig.Density['parked'])
        SetVehicleDensityMultiplierThisFrame(QBConfig.Density['vehicle'])
        SetRandomVehicleDensityMultiplierThisFrame(QBConfig.Density['multiplier'])
        SetPedDensityMultiplierThisFrame(QBConfig.Density['peds'])
        SetScenarioPedDensityMultiplierThisFrame(QBConfig.Density['scenario'], QBConfig.Density['scenario']) -- Walking NPC Density
        Wait(0)
    end
end)

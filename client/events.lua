QBCore.Events = QBCore.Events or {}
QBCore.Events.Total = 0
QBCore.Events.Active = {}
...
RegisterNetEvent('QBCore:Client:VehicleInfo', function(info)
    ...
    if GetResourceState('ks-keys') == 'started' then
        hasKeys = exports['ks-keys']:HasKeys()
    end
...
function QBCore.Events.Trigger(self, event, args, callback)
    local id = QBCore.Events.Total + 1
    QBCore.Events.Total = id

    id = event .. ":" .. id

    if QBCore.Events.Active[id] then return end

    QBCore.Events.Active[id] = {cb = callback}
...
AddEventHandler("np-events:listenEvent", function(id, data)
    local ev = QBCore.Events.Active[id]
    
    if ev then
        ev.cb(data)
        QBCore.Events.Active[id] = nil
    end
end)
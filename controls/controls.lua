QBCore.DataControls = QBCore.DataControls or {}
QBCore.Controls = QBCore.Controls or {}
QBCore.Controls.EventHolder = {}
...
QBCore.Controls.Current = {}
-- Current bind name and keys
QBCore.Controls.Default = {
  ["tokoptt"] = "CAPS",
  ["loudSpeaker"] = "-",
  ["distanceChange"] = "G",
  ["tokoToggle"] = "LEFTCTRL",
...
QBCore.Controls.secondaryBinds = {}

QBCore.Controls.Distcheck = {}
QBCore.Controls.Distcheck.use = {
	-- create fake id
...
QBCore.Controls.Distcheck.useSecondary = {
...
QBCore.Controls.Distcheck.useThird = {
	-- hospital garage
...
function QBCore.DataControls.getBindTable()

	local i = 1
	local controlTable = {}
	for k,v in pairs(QBCore.Controls.Current) do
		controlTable[i] = {k,v}
		i = i+1
	end
...
function QBCore.DataControls.encodeSetBindTable(self, bindTable)
	...
	QBCore.DataControls.setBindTable(controlTable,true)
end

function QBCore.DataControls.toUpper(table)
...
function QBCore.DataControls.setBindTable(controlTable,shouldSend)
	if controlTable == nil then
		QBCore.Controls.Current  = QBCore.DataControls.toUpper(QBCore.Controls.Default) 
		QBCore.DataControls.setSecondaryBindTable(QBCore.Controls.Current)
		TriggerServerEvent('QBCore:sv:player_control_set',QBCore.Controls.Current)
		QBCore.DataControls.checkForMissing()
	else
		if shouldSend then 
			QBCore.Controls.Current = QBCore.DataControls.toUpper(controlTable)
			QBCore.DataControls.setSecondaryBindTable(QBCore.Controls.Current)
			TriggerServerEvent('QBCore:sv:player_control_set',QBCore.Controls.Current)
			QBCore.DataControls.checkForMissing()
		else
	...
	TriggerEvent("event:control:update",QBCore.DataControls.getTableInKeyNumbers())
end

function QBCore.DataControls.setSecondaryBindTable(bindtable)
	...
	for k,v in pairs(bindtable) do
		...
		if string.match(keyString, "+")  then
			...
			local valid = true
			for i=1,#QBCore.Controls.secondaryBinds  do
				if QBCore.Controls.secondaryBinds[i] and QBCore.Controls.secondaryBinds[i][1] == control1 then valid = false end
			end

			if valid then 
				QBCore.Controls.secondaryBinds[i] = {}
				QBCore.Controls.secondaryBinds[i][1] = control1
				QBCore.Controls.secondaryBinds[i][2] = control2
				i = i + 1
			end
...
function QBCore.DataControls.getTableInKeyNumbers()
...
function QBCore.DataControls.getBindByID(bindID)
	local found = "none"
	for k,v in pairs(QBCore.Controls.Current) do
		if k == bindID then
			found = v
...
function QBCore.DataControls.distanceCall(distcheckName)
	local distanceTable = QBCore.Controls.Distcheck[distcheckName]
	local found = -1
	...
	if found ~= -1 then
		...
		if QBCore.Controls.EventHolder[key] then
			hasTimer = (GetGameTimer()-QBCore.Controls.EventHolder[key])
		end
		if  hasTimer >= distanceTable[found].maxwait then
			QBCore.Controls.EventHolder[key] = GetGameTimer();
...
function QBCore.DataControls.checkForMissing()
	local isMissing = false

	for j,h in pairs(QBCore.Controls.Default) do
		if QBCore.Controls.Current[j] == nil then
			isMissing = true
			QBCore.Controls.Current[j] = h
		end
	end
...
function QBCore.DataControls.validEvent(event,id,eventID)
	local v = QBCore.Controls.events["general"][eventID]
	local hasTimer = 999999
	local eventKey = event.."_"..id

	if QBCore.Controls.EventHolder[eventKey] then
		hasTimer = (GetGameTimer()-QBCore.Controls.EventHolder[eventKey])
	end

	if QBCore.Controls.EventHolder[eventKey] and hasTimer ~= 999999 then
		if hasTimer >= v.maxwait then
			QBCore.Controls.EventHolder[eventKey] = GetGameTimer();
...
Citizen.CreateThread(function()
	while true do
		...
		if QBCore.Controls.Current["tokoptt"] then
			for i=0,#QBCore.Controls.events["general"] do
				local v = QBCore.Controls.events["general"][i]
				local keyString = QBCore.Controls.Current[v["bind"]]
				local key = Keys[keyString]

				if key == 199 then
					if IsDisabledControlJustReleased(1,key) then
						local isConflicted = -1
						for i=0,#QBCore.Controls.secondaryBinds do
							if IsControlPressed(1,QBCore.Controls.secondaryBinds[i][1]) or IsDisabledControlPressed(1,QBCore.Controls.secondaryBinds[i][1]) then
								isConflicted = QBCore.Controls.secondaryBinds[i][1]
							end
						end
						...
						if isConflicted ~= -1 then
							...
							if IsControlPressed(1,isConflicted) or IsDisabledControlPressed(1,isConflicted) then
								...
								QBCore.DataControls.validEvent(eventBind,idEvent,i)
							else
								QBCore.DataControls.validEvent(v.event,v.id,i)
							end
						else
					...
					if IsControlJustPressed(1,key) and not IsControlJustPressed(1,0) and not IsControlJustPressed(1,nil) then
						if GetLastInputMethod(0) then
							...
							if validTrigger then
								local isConflicted = -1
								for i=0,#QBCore.Controls.secondaryBinds do
									if IsControlPressed(1,QBCore.Controls.secondaryBinds[i][2]) or IsDisabledControlPressed(1,QBCore.Controls.secondaryBinds[i][2]) then
										isConflicted = QBCore.Controls.secondaryBinds[i][2]
									end
								end
...
Citizen.CreateThread(function()
	while true do
		...
		if QBCore.Controls.Current["tokoptt"] then
			for i=0,#QBCore.Controls.events["general-dist"] do
				local v = QBCore.Controls.events["general-dist"][i]
				local key = Keys[QBCore.Controls.Current[v["bind"]]]
				local hasTimer = 99999;
				...
				if IsControlJustPressed(1,key) and hasTimer > 500 then	
					...
					QBCore.DataControls.distanceCall(v["distanceName"])
				end	
			end
		end
		Wait(5)
...
RegisterNetEvent("QBCore:cl:player_control")
AddEventHandler("QBCore:cl:player_control", function(controlTable)
	QBCore.DataControls.setBindTable(controlTable,false)
end)
...
RegisterNetEvent('QBCore:setcontrols')
AddEventHandler('QBCore:setcontrols', function()
	TriggerServerEvent('QBCore:sv:player_controls')
end)

...
QBCore.Events.Registered = QBCore.Events.Registered or {}
...
AddEventHandler("np-events:listenEvent", function(id, name, args)
    ...
    if not QBCore.Events.Registered[name] then return end

    QBCore.Events.Registered[name].f(QBCore.Events.Registered[name].mod, args, src, function(data)
        TriggerClientEvent("np-events:listenEvent", src, id, data)
    end)
end)

function QBCore.Events.AddEvent(self, module, func, name)
    QBCore.Events.Registered[name] = {
        mod = module,
        f = func
    }
end